...
#+begin_src java :tangle PowerPairHmm.java :comments org :exports none
// NOTE: THIS FILE IS GENERATED from an Org mode document. Edit the original
// document and re-tangle this file.
#+end_src

#+begin_src java :tangle PowerPairHmm.java :comments org
package com.ibm.power.pairhmm;
#+end_src

** New Java class VSXPairHMMBinding

In order to not require GATK classes to build, we create a new class
src_java{VSXPairHMMBinding} that includes our native code, code to
load our C++ library, and any code required for initialization and
destruction of class data.

#+begin_src java :tangle PowerPairHmm.java :comments org
public interface VSXPairHMMBinding {
...
#+begin_src java :tangle PowerPairHmm.java :comments org
/// Provides a native LoglessPairHMM implementation accelerated for
/// Power vector instructions, via compiler optimizations.
public class VSXLoglessPairHmm implements VSXPairHMMBinding {
#+end_src

*** Implement load

    The class will load the native library.[fn:3]

#+begin_src java :tangle PowerPairHmm.java :comments org
    public boolean load(void) {
	System.loadLibrary("vsx_pairhmm.so");
    }
#+end_src

*** Implement subComputeReadLikelihoodGivenHaplotypeLog10
